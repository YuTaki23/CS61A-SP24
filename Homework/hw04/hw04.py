...
def balanced(m):
    ...
    if is_planet(m):
        ...
        left_sum = length(left(m)) * total_mass(left_end)
        right_sum = length(right(m)) * total_mass(right_end)

        return left_sum == right_sum and balanced(left_end) and balanced(right_end)
